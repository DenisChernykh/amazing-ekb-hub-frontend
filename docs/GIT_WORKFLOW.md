...
2. `pre-push` hook проверяет имя ветки и запускает быстрый набор: `lint + typecheck + test:unit`.
3. `lint` включает:
   - запрет относительных внутренних импортов в `src/**` и `test/**`;
   - проверку TSDoc/документации по принятой политике.
4. CI на PR в `stage` и `main` запускает полный набор проверок, включая integration/e2e.
